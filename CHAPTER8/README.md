...
- **indexOf**
	...
	var arr = [2, 9, 9];
	arr.indexOf(2); // 0
	arr.indexOf(7); // -1 요소가 배열에 존재하지 않는다.
	```
- **lastIndexOf**
	- 배지정된 요소가 배열에서 발견 될 수있는 마지막 인덱스를 반환한다.
	- 일치하는 값을 찾지 못하면 -1을 반환한다.
	```js
	var arr = [2, 5, 9, 2];
	arr.lastIndexOf(2); // 3
	arr.lastIndexOf(7); // -1 요소가 배열에 존재하지 않는다.
	```
- **findIndex**
...
- 배열.filter((요소, 인덱스, 배열) => { return 요소 });
- 배열의 모든 요소를 변형할 때
- 데이터를 동일 함수에 반복적으로 적용
	- 데이터에 변형을 주는 연산(특정규칙을 적용시켜 새로운 배열을 만든다.)
