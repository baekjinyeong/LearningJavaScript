...
- **indexOf**
	- 배열에서 앞에서 부터 지정된 요소를 찾아 첫 번째 인덱스를 반환하고, 존재하지 않으면 -1을 반환한다.
- **lastIndexOf**
	- 배열에서 뒤에서 부터 지정된 요소를 찾아 첫 번째 인덱스를 반환하고, 존재하지 않으면 -1을 반환한다.
- **findIndex**
	- 호출 함수에 지정된 테스트 조건을 충족하는 첫 번째 배열 요소의 인덱스 값을 반환한다.
	- true를 반환할 때까지 배열의 각 요소에 대해 한 번씩 오름차순으로 호출 함수를 호출한다.
